...
parser.add_argument('--train', default=True, type=lambda x: bool(distutils.util.strtobool(x)))
# Training parameter
...
# Dataset parameters
parser.add_argument('--datasetroot', type = str, default = '/home/jason/Data/Places2/data_large', help = 'the training folder: val_256, test_large, data_256')
parser.add_argument('--datasetname', type = str, default = '')

parser.add_argument('--imgsize', type = int, default = 512, help = 'size of image')

parser.add_argument('--dist_url', type= str, default='/home/jason/Data/')

opt = parser.parse_args()

    ##这样直接用SLURM控制进程数量
    ##如果要让torch自己控制就要让nprocs=ngpus_per_node
    #ngpus_per_node = torch.cuda.device_count()
    #opt.local_rank = int(os.environ["SLURM_LOCALID"])
    #opt.rank = int(os.environ["SLURM_PROCID"])
    #opt.world_size = int(os.environ["SLURM_NPROCS"])
    #opt.job_id = int(os.environ["SLURM_JOBID"])
